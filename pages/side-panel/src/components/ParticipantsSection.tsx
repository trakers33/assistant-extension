...
export const ParticipantsSection = ({ participants, isLight }: ParticipantsSectionProps) => {
    ...
    return (
        <div className={`participants px-4 py-4 ${isLight ? 'bg-white' : 'bg-gray-900'}`} role="tabpanel" id="participants-panel">
            {(() => {
                const { outsideOrg, withinOrg } = getOrganizationParticipants();
                return (
                    <>
                        <div className="mb-8">
                            {participants.filter(p => p.humanized_status === 'in_meeting').map(participant => (
                                <div
                                    key={participant.deviceId}
                                    className={`participant flex items-center gap-4 mb-4 p-3 rounded-lg transition-colors ${
                                        isLight 
                                            ? 'hover:bg-gray-50' 
                                            : 'hover:bg-gray-800'
                                    }`}>
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium shadow-sm overflow-hidden ${
                                        isLight 
                        ...
                        {withinOrg.length > 0 && (
                            <div>
                                ...
                                {withinOrg.map(participant => (
                                    <div
                                        key={participant.deviceId}
                                        className={`participant flex items-center gap-4 p-3 rounded-lg transition-colors ${
                                            isLight 
                                                ? 'hover:bg-gray-50' 
                                                : 'hover:bg-gray-800'
                                        }`}>
